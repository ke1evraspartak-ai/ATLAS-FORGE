...
>
  Аналитика
        ...
        {dashboardOverdueTasks.length > 0 && (
          <Link
            href="/manager/tasks"
            className="block mt-8 bg-red-500/15 border border-red-500 text-red-300 rounded-3xl p-5 font-bold"
          >
            ⚠ Просроченных задач: {dashboardOverdueTasks.length}
          </Link>
        )}
        ...
        <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-6 mt-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
            <div className="text-zinc-500">Просрочено</div>
            <div className="text-5xl font-black text-red-500 mt-3">
              {dashboardOverdueTasks.length}
            </div>
          </div>
        </div>
        ...
        <div className="grid xl:grid-cols-3 gap-8 mt-8">
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
            <div className="flex items-center justify-between gap-4">
              <h2 className="text-2xl font-black">Задачи на сегодня</h2>
              <Link href="/manager/tasks" className="text-orange-500 hover:underline font-bold">
                Все задачи
              </Link>
            </div>

            <div className="mt-5 space-y-3">
              {dashboardTasksToday.slice(0, 5).map((task) => {
                const client = clients.find((item) => item.id === task.client_id);

                return (
                  <button
  key={task.id}
  onClick={() => window.location.href = `/manager/clients?client=${task.client_id}`}
...
>
              ...
              {dashboardTasksToday.length === 0 && (
                <div className="text-zinc-500">На сегодня задач нет.</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <Link href="/manager/leads" className="text-orange-500 hover:underline font-bold">
                Все заявки
              </Link>
            </div>

            <div className="mt-5 space-y-3">
              {dashboardRecentRequests.map((offer) => (
                <button
                  key={offer.id}
                  onClick={() => {
  localStorage.setItem("openOfferId", offer.id);
  localStorage.setItem("managerOpenOfferId", offer.id);
  window.location.href = `/manager?offerId=${offer.id}`;
}}
                  className="w-full text-left bg-green-500/10 border border-zinc-800 hover:border-orange-500 transition rounded-2xl p-4"
                >
                  <div className="font-bold">{offer.client_name || "Без клиента"}</div>
                  <div className="text-zinc-500 text-sm mt-1">
                    {offer.city || "Без города"} · {offer.offer_number}
                  </div>
                </button>
              ))}

              {dashboardRecentRequests.length === 0 && (
                <div className="text-zinc-500">Заявок пока нет.</div>
              )}
            </div>
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
            <div className="flex items-center justify-between gap-4">
              <h2 className="text-2xl font-black">Последние КП</h2>
              <button
    ...
    <span className="bg-zinc-900 border border-zinc-700 rounded-full px-3 py-1">
      {formatDate(offer.created_at)}
              ...
              {dashboardRecentOffers.length === 0 && (
                <div className="text-zinc-500">КП пока нет.</div>
              )}
            </div>
          </div>
        </div>
